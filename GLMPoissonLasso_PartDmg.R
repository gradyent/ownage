...
df_tycoon <- dplyr::select(df_tycoon, -x_pos, -y_pos)
...
df_train_comp <- dplyr::select(df_train, -admin_L3_name, -comp_damage_houses, -total_damage_houses)
...
v_exclude <- c("typhoon_name","admin_L3_code","admin_L2_code","part_damage_houses")
glm_form <- as.formula(paste("part_damage_houses~", paste(n[!n %in% v_exclude], collapse="+")))
...
for(j in 1:length(lambda)){
  ...
  glm3 <- try(glmmLasso(glm_form,rnd = list(typhoon_name=~1), 
                    ...
                    lambda=lambda[j], switch.NR=F,final.re=F,
  ...
  cor1[j] <- cor(df_test$part_damage_houses, predicted_test)
}
...
plot(df_test$part_damage_houses, predicted_test)
cor(df_test$part_damage_houses, predicted_test)

plot(df_train$part_damage_houses, predicted_train)
cor(df_train$part_damage_houses, predicted_train)
